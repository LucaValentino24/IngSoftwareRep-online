...
public final class ImageObject extends AbstractGraphicObject {
	...
	public String getProp() {
		return "Immagine -> id: " + getId() + ", posizione: [" + position.getX() + ","+ position.getY() + "], dim: ["
				+ getDimension().getWidth() + "," + getDimension().getHeight() + "]";
	}

	@Override
	public double getPerimeter() {
		return 2*getDimension().getWidth() + 2*getDimension().getHeight();
	}

	@Override
	public double getArea() {
		return getDimension().getWidth()*getDimension().getHeight();
	}
}
